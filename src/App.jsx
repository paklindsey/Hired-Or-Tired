...
function computeStats(apps) {
  ...
  const thisWeek = apps.filter(
    (a) => a.date && new Date(a.date) >= oneWeekAgo,
  ).length;
...
export default function App() {
  ...
  useEffect(() => {
    if (guestMode) {
      const stored = sessionStorage.getItem("hot_guest_apps");
  ...
  async function addApp(e) {
    e.preventDefault();
    if (guestMode) {
      const newApp = { ...form, id: crypto.randomUUID() };
      saveGuestApps([newApp, ...apps]);
      setForm(EMPTY_FORM);
      setShowAdd(false);
      return;
    }
    const { data, error } = await supabase
      .from("applications")
      .insert([{ ...toRow(form), user_id: session.user.id }])
      .select()
      .single();
    if (error) {
  ...
  async function deleteApp(id) {
    if (guestMode) {
      saveGuestApps(apps.filter((a) => a.id !== id));
      setSelected(null);
      return;
    }
  ...
  async function saveEdit(e) {
    e.preventDefault();
    if (guestMode) {
      saveGuestApps(apps.map((a) => (a.id === selected.id ? editForm : a)));
      setSelected(null);
      setEditForm(null);
      return;
    }
    ...
    <div className="app">
        ...
        {guestMode ? (
          <div className="profile-menu">
            <button
              ...
              onClick={() => {
                setGuestMode(false);
                sessionStorage.removeItem("hot_guest_apps");
              }}
            >
              Sign In
            </button>
      ...
      <div className="stats-grid">
        <StatCard
          label="Applied:"
          value={stats.total}
          delta={stats.thisWeek > 0 ? `+${stats.thisWeek} this week` : null}
        />
              ...
              {apps.length === 0 ? (
                ...
                apps.map((app) => (
                  <tr key={app.id} onClick={() => openDetail(app)}>
                    ...
                    <td>
                      {app.priority ? (
                        <span className={priorityBadgeClass(app.priority)}>
                          {app.priority}
                        </span>
                      ) : (
                        "—"
                    ...
                    <td>
                      {app.referenceUrl ? (
                        ...
                        >
                          {app.reference || app.referenceUrl}
                        </a>
                      ) : (
                        app.reference || "—"
                      )}
      ...
      {showAdd && (
        ...
        >
          <div className="modal-tabs">
            <button
              className={`modal-tab${addTab === "smart" ? " active" : ""}`}
              type="button"
              ...
              col.map((app) => (
                ...
                >
                  ...
                  <div className="kanban-card-footer">
                    <span className="kanban-card-salary">
                      {app.salary || "—"}
                    </span>
                    {app.priority && (
                      <span className={priorityBadgeClass(app.priority)}>
  ...
  return (
    <div className="form-grid">
      ...
      <div className="form-group full">
        ...
        <textarea
          className="form-input job-description-area"
...
function SmartPaste({ onParsed }) {
  ...
  async function handleExtract() {
    ...
    try {
      ...
      setError(
        err?.message ||
          "Could not extract details. Check your API key or try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleUrlExtract() {
    ...
    try {
      ...
      if (fnError) throw new Error(fnError.message || "Failed to fetch URL");
      ...
      setError(
        err?.message ||
          "Could not fetch or parse the URL. Try pasting the text instead.",
      );
    } finally {
      setLoading(false);
    }
      ...
      {smartTab === "paste" ? (
        <>
          <p className="smart-paste-hint">
            Paste a job description below — AI will extract the details for you.
          </p>
          <textarea
        ...
        <>
          <p className="smart-paste-hint">
            Paste a job posting URL — AI will fetch and extract the details for
            you.
          </p>
          <input
            className="form-input"
  ...
  return (
    <div className="login-screen">
      ...
      <div className="login-card">
        ...
        {signedUp && (
          <p className="login-success">Account created! Sign in below.</p>
        )}
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <p className="smart-paste-error" style={{ marginTop: 8 }}>
              {error}
            </p>
          )}
          ...
          <div className="login-card-footer">
            ...
            <button type="button" className="login-text-btn" onClick={onGuest}>
              Continue as Guest
            </button>
          </div>
        </form>
